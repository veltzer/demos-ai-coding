...
"""
Stage 5: collect the three booking slots and "buy" the ticket.

Selling a ticket needs a departure city, a destination city and a date. This is
the first stage to keep a conversation history: the API is stateless, so the
whole exchange is resent each turn and the model re-derives the slots from it.
Once all three are filled the ticket is bought and the history is reset.
